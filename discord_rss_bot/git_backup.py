...
def setup_backup_repo(backup_path: Path) -> bool:
    ...
    try:
        ...
        if not git_dir.exists():
            subprocess.run(["git", "init", str(backup_path)], check=True, capture_output=True)  # noqa: S603, S607
        ...
        for key, value in (("user.email", "discord-rss-bot@localhost"), ("user.name", "discord-rss-bot")):
            result = subprocess.run(  # noqa: S603, S607
                ["git", "-C", str(backup_path), "config", "--local", key],
                capture_output=True,
            )
            if result.returncode != 0:
                subprocess.run(  # noqa: S603, S607
                    ["git", "-C", str(backup_path), "config", "--local", key, value],
                    check=True,
                    capture_output=True,
                )
    except Exception:
...
def export_state(reader: Reader, backup_path: Path) -> None:
    ...
    for feed in reader.get_feeds():
        ...
        for tag in _FEED_TAGS:
            try:
                value = reader.get_tag(feed, tag, None)
...
def commit_state_change(reader: Reader, message: str) -> None:
    ...
    try:
        ...
        subprocess.run(["git", "-C", str(backup_path), "add", "-A"], check=True, capture_output=True)  # noqa: S603, S607

        # Only create a commit if there are staged changes.
        diff_result = subprocess.run(  # noqa: S603, S607
            ["git", "-C", str(backup_path), "diff", "--cached", "--exit-code"],
            capture_output=True,
        )
        ...
        subprocess.run(  # noqa: S603, S607
            ["git", "-C", str(backup_path), "commit", "-m", message],
            check=True,
            capture_output=True,
        )
        logger.info("Committed state change to backup repo: %s", message)

        remote: str = get_backup_remote()
        if remote:
            subprocess.run(  # noqa: S603, S607
                ["git", "-C", str(backup_path), "push", remote, "HEAD"],
                check=True,
                capture_output=True,
            )
            logger.info("Pushed state change to remote %s: %s", remote, message)
    except Exception:
